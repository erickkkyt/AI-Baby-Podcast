...
export default function PaymentSuccessPage() {
  ...
  useEffect(() => {
    async function processPayment() {
      try {
        const paramsToSend: Record<string, string> = {}; // 明确类型为 string
        
        // 直接从 searchParams 构建对象，只包含实际存在的参数
        searchParams.forEach((value, key) => {
          paramsToSend[key] = value; 
        });

        // 验证必要参数 (signature 是必须的，其他的 Creem 会校验)
        // Creem文档通常要求 checkout_id 和 order_id 也存在于回调中
