...
export default function Pricing() {
  ...
  const plans = [
    ...
    },
    {
      ...
      ctaLink: '/subscribe?plan=creator', 
      highlight: true,
    },
    {
      name: 'Pro Plan',
    ...
    <section id="pricing" className="py-20 bg-gray-50">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">BabyPodcast Creation Plans</h2>
          ...
          <div className="mt-8 flex items-center justify-center">
            ...
            {annual && <span className="ml-2 rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-800">Save 20%</span>} {/* Discount info can be kept or adjusted */}
          </div>
        </div>
            ...
            >
              ...
              <div className={`pt-8 ${plan.highlight ? 'pb-6' : 'pb-8'} px-6 ${plan.highlight ? 'pt-12' : ''}`}>
                <div className="text-center">
                  ...
                  <p className="text-gray-500 mb-6 h-10">{plan.description}</p> {/* 固定高度防止抖动 */}
                  ...
                  {annual && plan.annualPrice !== plan.monthlyPrice && parseInt(plan.annualPrice) > 0 && (
                    <p className="mt-1 text-sm text-gray-500">Annual billing ${parseInt(plan.annualPrice, 10) * 12}</p>
                  )}
                  {plan.monthlyPrice === '0' && (
              ...
              <div className="border-t border-gray-100 bg-gray-50 px-6 py-6">
                <ul className="space-y-4 h-48"> {/* 固定特性列表高度 */}
